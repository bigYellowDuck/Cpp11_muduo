...
char require_32_bit_integer_at_list[sizeof(int) >= sizeof(int32_t) ? 1 : -1];

int getJulianDayNumber(int year, int month, int day)
{
    (void) require_32_bit_integer_at_list; // no warning please;
    int a = (14 - month) / 12;
    int y = year + 4800 - a;
    int m = month + 12 * a -3;
...
Date::Date(int y, int m , int d)
    : _julianDayNumber(getJulianDayNumber(y, m, d))
{
}

Date::Date(const struct tm& t)
    : _julianDayNumber(getJulianDayNumber(
            t.tm_year + 1900,
            t.tm_mon + 1,
            t.tm_mday))
{
...
std::string Date::toIsoStirng() const 
{
    char buf[32];
