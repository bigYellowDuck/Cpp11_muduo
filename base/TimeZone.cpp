...
class File : public Noncopyable
{
 public:
    ...
    explicit operator bool() const { return _fp; }
...
const Localtime* findLocaltime(const TimeZone::Data& data, Transition sentry, Comp comp)
{
    ...
    {
        auto trans = std::lower_bound(data.transitions.begin(), 
                                 data.transitions.end(),
                                 sentry,
                                 comp);
        if (trans != data.transitions.end())
        {
            if (!comp.equal(sentry, *trans))
            {
                assert(trans != data.transitions.begin());
                --trans;
            }
            local = &data.localtimes[trans->localtimeIdx];
        }
        else 
        {
